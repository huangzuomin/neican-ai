...
def inspect_status(db_path: Path = DB_PATH, min_fetch_interval_minutes: int = 120) -> HeartbeatStatus:
    ...
    if not db_path.exists():
        return HeartbeatStatus(True, "db_missing", None, 0, 0, 0)
    ...
    if pending_raw_items > 0:
        return HeartbeatStatus(True, "pending_raw_items", recent_fetch_age_minutes, pending_raw_items, pending_events, pending_decisions)
    if pending_events > 0:
        return HeartbeatStatus(True, "pending_events", recent_fetch_age_minutes, pending_raw_items, pending_events, pending_decisions)
    if pending_decisions > 0:
        return HeartbeatStatus(True, "pending_decisions", recent_fetch_age_minutes, pending_raw_items, pending_events, pending_decisions)
    if recent_fetch_age_minutes is None:
        return HeartbeatStatus(True, "no_previous_fetch", None, pending_raw_items, pending_events, pending_decisions)
    if recent_fetch_age_minutes >= min_fetch_interval_minutes:
        return HeartbeatStatus(True, "fetch_interval_elapsed", recent_fetch_age_minutes, pending_raw_items, pending_events, pending_decisions)
    return HeartbeatStatus(False, "up_to_date", recent_fetch_age_minutes, pending_raw_items, pending_events, pending_decisions)
