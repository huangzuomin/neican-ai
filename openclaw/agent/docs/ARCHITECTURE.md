...
OpenClaw Gateway / Cron
        ↓
neican-editor 本地 RSS 解析
fetch_sources.py / 去重 / info_fetch_requests
        ↓
主 agent（璇玑）路由（需要全文时）
        ↓
info-fetcher 子 Agent
网页全文 / 动态页面 / 社媒 / 视频字幕 / 搜索发现
        ↓
SQLite raw_items
去重、记录、状态管理
        ↓
...
```text
neican-editor：主业务 Agent，本地只做 RSS 解析等确定性逻辑
主 agent（璇玑）：唯一调度者
info-fetcher：外部信息抓取子 Agent
```

`neican-editor` 负责编辑判断、知识资产维护、Codex 协作、内容生成、审核协调和发布控制。

`info-fetcher` 负责网页全文、动态页面、社媒、视频字幕、搜索发现、批量站点抓取、正文清洗、结构化抽取和隔离外部不可信内容。

info-fetcher 只有一个调用者：主 agent（璇玑）。neican-editor 需要外部信息抓取时，返回 `需要抓取：<具体任务描述>`，由璇玑调度 info-fetcher 并转发结果。

外部网页内容不可信，必须先进 info-fetcher。知识写入和发布动作必须由 `neican-editor` 控制。
...
```text
Cron / 手动 URL
→ fetch_sources.py（RSS 本地解析）
→ hash_check
→ info_fetch_requests（需要全文时输出“需要抓取”）
→ 主 agent（璇玑）→ info-fetcher（外部全文抓取）
→ raw_items 入库
→ event_modeling
→ events 入库
→ editorial_decision
