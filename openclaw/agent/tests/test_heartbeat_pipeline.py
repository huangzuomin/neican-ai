...
def test_inspect_status_includes_pending_info_fetch_requests(tmp_path):
    db_path = init_temp_db(tmp_path)
    with get_conn(db_path) as conn:
        conn.execute(
            """
            INSERT INTO info_fetch_requests (source_url, title, reason, route_message, status)
            VALUES (
              'https://example.com/full',
              'Needs full text',
              'rss_summary_only',
              '需要抓取：请抓取以下 URL 的全文：https://example.com/full',
              'pending'
            )
            """
        )
    ...
    assert status.should_run is True
    assert status.reason == "pending_info_fetch_requests"
    assert status.pending_info_fetch_requests == 1
...
def test_heartbeat_main_json_path(monkeypatch, capsys):
    monkeypatch.setattr(
        ...
        lambda **kwargs: HeartbeatResult(
            True,
            {"should_run": True, "reason": "pending_events", "recent_fetch_age_minutes": 180, "pending_raw_items": 0, "pending_events": 2, "pending_decisions": 0, "pending_info_fetch_requests": 0, "info_fetch_route_message": None},
            {"date": "2026-05-01"},
        ),
    )
...
def test_heartbeat_prints_info_fetch_route_message(monkeypatch, capsys):
    route_message = "需要抓取：请抓取以下 URL 的全文：\n- https://example.com/full"
    monkeypatch.setattr(
        "heartbeat_pipeline.heartbeat_run",
        lambda **kwargs: HeartbeatResult(
            False,
            {"should_run": True, "reason": "pending_info_fetch_requests", "recent_fetch_age_minutes": 10, "pending_raw_items": 0, "pending_events": 0, "pending_decisions": 0, "pending_info_fetch_requests": 1, "info_fetch_route_message": route_message},
            None,
        ),
    )
    from heartbeat_pipeline import main

    monkeypatch.setattr(sys, "argv", ["heartbeat_pipeline.py"])
    main()
    out = capsys.readouterr().out
    assert route_message in out
