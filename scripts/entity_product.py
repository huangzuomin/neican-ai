...
def export_hugo(conn, site_dir: Path = SITE_DIR) -> int:
    ensure_schema(conn)
    rows = conn.execute("SELECT * FROM entity_profiles WHERE status='public' ORDER BY related_events DESC, name").fetchall()
...
def write_entity_page(base: Path, row) -> None:
    ...
    body = [
        ...
        f"<h1>{row['name']}</h1>",
        f"<p class=\"page-lead\">{row['summary'] or ''}</p>",
    ...
    if events:
        ...
        for ev in events[:20]:
            ...
            date = ev.get("date") or ""
            body.append(f"<article><time>{date}</time><h3>{f'<a href={url}>{title}</a>' if url else title}</h3><p>{ev.get('summary','')}</p></article>")
