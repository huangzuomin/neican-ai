...
def insert_event(
    ...
    with get_conn(db_path) as conn:
        conn.execute(
            """
            INSERT INTO raw_items (source_url, title, content_hash, status)
            VALUES (?, ?, ?, 'processed')
            """,
            (f"https://example.com/{title}", title, f"hash-{title}"),
        )
...
def test_make_decisions_assigns_a_b_c_d_and_review_queue(tmp_path):
    ...
    a_event = insert_event(db_path, "A event", 80, 70, 80, 10, 0.80)
    b_event = insert_event(db_path, "B event", 50, 20, 45, 10, 0.70)
...
def test_make_decisions_sends_high_risk_to_review_queue(tmp_path):
    db_path = init_temp_db(tmp_path)
    event_id = insert_event(db_path, "Risk event", 80, 70, 80, 75, 0.80)
...
def test_make_decisions_dry_run_does_not_write(tmp_path):
    db_path = init_temp_db(tmp_path)
    insert_event(db_path, "A event", 80, 70, 80, 10, 0.80)
