...
def select_raw_items(conn, raw_item_id: int | None, limit: int | None):
    sql = """
        SELECT
          ...
          raw_items.extraction_confidence,
        ...
        for row in rows:
            ...
            try:
                ...
                if extraction_confidence is not None and float(extraction_confidence) < 0.5:
                    message = f"extraction_confidence={float(extraction_confidence):.2f} below threshold=0.50"
                    conn.execute(
                        "UPDATE raw_items SET status = 'needs_refetch', error_message = ? WHERE id = ?",
                        (message, raw_item_id_value),
                    )
