...
def load_rules(config_dir: Path) -> dict[str, Any]:
    ...
    trust_path = Path(config_dir) / "source_trust.yaml"
    if trust_path.exists():
        rules["source_trust"] = yaml.safe_load(trust_path.read_text(encoding="utf-8")) or {}
    return rules
...
def select_events(conn, event_id: int | None, limit: int | None):
    sql = """
        SELECT events.*, COALESCE(sources.trust_level, 3) AS source_trust_level
        FROM events
...
# AI-related topic slugs from taxonomy.yaml — events without any of these are auto-D.
_AI_TOPIC_SLUGS = frozenset({
    "ai-agents", "vibe-coding", "rag", "mcp", "ai-search",
    "embodied-ai", "llm", "multimodal", "ai-safety", "ai-policy",
})


def _has_ai_topic(topics: list[Any]) -> bool:
    """Check whether any assigned topic is in the AI taxonomy."""
    for t in topics:
        slug = (t if isinstance(t, str) else (t.get("slug") or t.get("name") or "")) if isinstance(t, dict) else str(t)
        if slug.lower().strip() in _AI_TOPIC_SLUGS:
            return True
    return False


def source_tier(trust_level: int, source_trust: dict[str, Any]) -> str:
    tiers = source_trust.get("tiers") or {}
    ordered = sorted(
        ((str(name), int(cfg.get("min_trust_level", 0))) for name, cfg in tiers.items()),
...
def tier_rank(tier: str) -> int:
    return {"C": 1, "B": 2, "A": 3, "S": 4}.get(tier, 0)
...
def decide(event, rules: dict[str, Any]) -> Decision:
    ...
    source_trust = rules.get("source_trust") or {}
    high_risk_trust = source_trust.get("high_risk") or {}
    event_source_tier = source_tier(int(event["source_trust_level"] or 3), source_trust)

    # Non-AI content filter: events without any AI-related topic are auto-D
    if not _has_ai_topic(topics):
        return Decision("D", d_rules.get("action", "ignore"), False, None, "no AI topic matched — non-AI content")
    ...
    if (
        ...
        high_risk_types = {str(item) for item in high_risk_trust.get("event_types", [])}
        min_tier = str(high_risk_trust.get("min_a_grade_tier", "A"))
        if event_type in high_risk_types and tier_rank(event_source_tier) < tier_rank(min_tier):
            return Decision(
                str(high_risk_trust.get("weak_source_grade", "C")),
                str(high_risk_trust.get("weak_source_action", "review_required")),
                True,
                str(high_risk_trust.get("review_type", "source_trust_review")),
                f"source trust tier {event_source_tier} below {min_tier} for high-risk A candidate",
            )
        return Decision(
            "A",
