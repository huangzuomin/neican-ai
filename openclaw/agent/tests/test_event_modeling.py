...
def test_model_events_marks_low_confidence_items_for_refetch(tmp_path):
    db_path = init_temp_db(tmp_path)
    raw_item_id = insert_source_and_raw_item(
        db_path,
        "OpenAI releases GPT model update",
    ...
    with get_conn(db_path) as conn:
        conn.execute(
            "UPDATE raw_items SET extraction_confidence = 0.3 WHERE id = ?",
            (raw_item_id,),
        )
    ...
    with get_conn(db_path) as conn:
        raw_item = conn.execute(
            "SELECT status, error_message FROM raw_items WHERE id = ?",
            (raw_item_id,),
        ).fetchone()
        request = conn.execute(
            "SELECT source_url, raw_item_id, reason, status FROM info_fetch_requests"
        ).fetchone()
    assert raw_item["status"] == "needs_refetch"
    assert "extraction_confidence" in raw_item["error_message"]
    assert request["raw_item_id"] == raw_item_id
    assert request["reason"] == "low_extraction_confidence"
    assert request["status"] == "pending"


def test_model_events_skips_low_ai_relevance_items(tmp_path):
    db_path = init_temp_db(tmp_path)
    raw_item_id = insert_source_and_raw_item(
        db_path,
        "港交所拟重启黄金期货交易",
        "港交所拟重启黄金期货交易，相关品种受到宏观市场关注。",
        trust_level=5,
    )

    result = model_events(db_path=db_path, config_dir=CONFIG_DIR, mock=True, raw_item_id=raw_item_id)

    assert result == EventModelingResult(modeled_count=0, failed_count=0, skipped_irrelevant_count=1)
    with get_conn(db_path) as conn:
        raw_item = conn.execute("SELECT status, error_message FROM raw_items WHERE id = ?", (raw_item_id,)).fetchone()
        assert conn.execute("SELECT COUNT(*) FROM events").fetchone()[0] == 0
    assert raw_item["status"] == "ignored"
    assert "ai_relevance_score" in raw_item["error_message"]


def test_model_events_scores_risk_keywords_and_respects_limit(tmp_path):
    db_path = init_temp_db(tmp_path)
    first_id = insert_source_and_raw_item(db_path, "AI funding round", "AI company announced funding for an LLM product.")
    second_id = insert_source_and_raw_item(db_path, "AI policy issue", "This AI policy includes 安全漏洞 and 法律诉讼.")
...
def test_model_events_normalizes_entities_before_insert(tmp_path, monkeypatch):
    db_path = init_temp_db(tmp_path)
    raw_item_id = insert_source_and_raw_item(
        db_path,
        "Open AI launches GPT-5",
        "Open AI announced a GPT-5 model update.",
    )
    ...
    def fake_llm_model_event(title, clean_text, client, model):
        return {
            ...
            "event_summary": clean_text,
            "event_type": "model_release",
            "entities": [{"name": "Open AI", "slug": "open-ai", "type": "company"}],
            "topics": ["llm"],
            "importance_score": 88,
            "seo_value_score": 66,
            "knowledge_value_score": 77,
    ...
    result = model_events(db_path=db_path, config_dir=CONFIG_DIR, mock=False, raw_item_id=raw_item_id)

    assert result == EventModelingResult(modeled_count=1, failed_count=0)
    with get_conn(db_path) as conn:
        event = conn.execute("SELECT entities_json FROM events WHERE raw_item_id = ?", (raw_item_id,)).fetchone()

    entities = json.loads(event["entities_json"])
    assert entities == [{"name": "OpenAI", "slug": "openai", "type": "company"}]
