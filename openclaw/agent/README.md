...
`heartbeat_pipeline.py` 会先检查是否存在待处理 raw_items / modeled events / pending decisions / info_fetch_requests，或是否已经超过 4 小时抓取间隔；只有需要时才继续调用 `pipeline.py`。

部署后的 OpenClaw 自动运行策略见 `RUN_STRATEGY.md`。测试工作流阶段允许用审核 skill 代替人工审核；审核通过并且 `bash scripts/validate.sh` 通过后，才允许推送 GitHub 发布仓库。
