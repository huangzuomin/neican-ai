...
Information Fetching Boundary:
neican-editor 不直接 spawn 或调用 info-fetcher。info-fetcher 只有一个调用者：主 agent（璇玑）。neican-editor 需要外部信息抓取时，只输出 `需要抓取：<具体任务描述>`，由璇玑调度 info-fetcher 并转发结果。

Local Fetching Boundary:
`scripts/fetch_sources.py` 仅保留 RSS 解析、来源 upsert、去重和 raw_items 入库等确定性逻辑。`--full-text` 不做本地 HTTP 全文抓取，而是把摘要型 RSS 条目写入 `info_fetch_requests` 并输出路由信号。
