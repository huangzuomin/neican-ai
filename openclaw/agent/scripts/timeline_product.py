...
def why_it_matters(row, entities: list[Any], topics: list[Any], claims: list[Any]) -> str:
    ...
    if row["decision_grade"] == "A":
        parts.append("具备较强的长期跟踪价值")
    elif row["decision_grade"] == "B":
        parts.append("适合作为趋势信号持续观察")
    if claims:
        valid_claims = [_claim_text(c) for c in claims if _claim_text(c)]
        if valid_claims:
            parts.append(f"包含 {len(valid_claims)} 条可追溯关键信息")
...
def write_year_page(site_dir: Path, year: str, rows: list[Any]) -> None:
    ...
    fm = {
        ...
        "seo": {"description": f"{year} 年 AI 行业关键事件、实体、主题与关键信息时间线。"},
    }
    body = [
        "<div class=\"timeline-product-page\">",
        f"<p class=\"eyebrow\">{year} Timeline</p>",
        f"<h1>{year} 年 AI 行业时间线</h1>",
        f"<p class=\"page-lead\">本页由 neican.ai 从事件库自动生成，共收录 {len(rows)} 个公开时间线节点。</p>",
    ...
    for month in sorted(months, reverse=True):
        ...
        for row in months[month]:
            body.append(
                ...
                f"<time>{row['date']}</time><div><p><span class=\"chip\">{row['event_type']}</span></p>"
...
def write_index_page(site_dir: Path, rows: list[Any], tracks: list[dict[str, Any]]) -> None:
    ...
    body = [
        ...
        f"<div><strong>{grade_counts.get('A', 0)}</strong><span>重点事件</span></div>",
        f"<div><strong>{grade_counts.get('B', 0)}</strong><span>趋势信号</span></div>",
